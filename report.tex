...
\section{Description}
    ...
    valid inputs before calling set\_process\_priority(pid, priority).  The
    ...
    \begin{algorithm}
        ...
        \begin{algorithmic}[1]
            ...
            \State $envelope \gets NULL$
            \Comment{Envelope for message passing}\\

            \Function{processSetMessage}{$message$}
                \State $index \gets 2$
                \Comment {Index of current position}
                \State $numLength \gets 0$
                \Comment {Length of parameters}
                \State $pid \gets 0$
                \State $priority \gets 0$\\

                \If {$message \lbrack index \rbrack \ne space$}
                    \State \Return $EINVAL$
                \EndIf\\

                \State $numLength, pid \gets \Call {read\_uint32}{message +
                    index}$\\
                ...
                \State $index \gets index + numLength$\\

                \If {$message \lbrack index \rbrack \ne space$}
                    \State \Return $EINVAL$
                \EndIf\\

                \State $numLength, priority \gets \Call{read\_uint32}{message +
                    index}$\\

                \If {$numLength = 0 \mid\mid priority > 255$}
                    \State \Return $EINVAL$
                \EndIf\\

                \State \Return \Call {set\_process\_priority}{$pid, priority$}
            \EndFunction\\

            \algstore{userSetPrioritySplit}
        \end{algorithmic}
    \end{algorithm}

    \begin{algorithm}
        \begin{algorithmic}[1]
            \algrestore{userSetPrioritySplit}
    ...
    \begin{algorithm}
        ...
        \label{code:read_uint_32}
        \begin{algorithmic}[1]

            \Function{read\_uint32}{$buf, bufLen, out$}
                ...
                \While {$read < bufLen$}
                    ...
                    \State $number \gets number + \Call {to\_int}{buf}$
                    \State $read \gets read + 1$
                    \State $buf \gets buf + 1$
                \EndWhile\\

                \If {$out \ne NULL$}
                    \State $out \gets number$
                \EndIf\\
